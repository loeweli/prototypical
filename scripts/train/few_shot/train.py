...
def main(opt):
    ...
    # save opts
    with open(os.path.join(opt['log.exp_dir'], 'opt.json'), 'w') as f: #results/opt.json
        json.dump(opt, f)
        f.write('\n')

    trace_file = os.path.join(opt['log.exp_dir'], 'trace.txt') #results/trace.txt

    # Postprocess arguments
    opt['model.x_dim'] = list(map(int, opt['model.x_dim'].split(','))) # 【1，28，28】
    opt['log.fields'] = opt['log.fields'].split(',') #loss,acc
    ...
    def on_start(state):
        if os.path.isfile(trace_file): # 删除trace.txt
            os.remove(trace_file)
        state['scheduler'] = lr_scheduler.StepLR(state['optimizer'], opt['train.decay_every'], gamma=0.5) # 调整学习率
