    ...
    if d['class'] not in OMNIGLOT_CACHE:
        ...
        image_ds = TransformDataset(ListDataset(class_images),
                                    compose([partial(convert_dict, 'file_name'),
                                             partial(load_image_path, 'file_name', 'data'), # open image
                                             partial(rotate_image, 'data', float(rot[3:])),  # rotate image
                                             partial(scale_image, 'data', 28, 28), # resize
...
def extract_episode(n_support, n_query, d):
    ...
    xs = d['data'][support_inds] # support set
    xq = d['data'][query_inds] # query set
...
def load(opt, splits):
    ...
    for split in splits:
        ...
        if opt['data.sequential']:
            ...
            sampler = EpisodicBatchSampler(len(ds), n_way, n_episodes) # 4112,60,100
